...
public class Main extends JFrame implements MouseListener
{
  ...
  //Function to check if the king will be in danger if the given move is made
    private boolean willkingbeindanger(Cell fromcell,Cell tocell)
    {
		...
		newboardstate[tocell.x][tocell.y].setPiece(newboardstate[fromcell.x][fromcell.y].getpiece(), false);
    ...
    private ArrayList<Cell> filterdestination (ArrayList<Cell> destlist, Cell fromcell)
    {
    	...
    	while (it.hasNext())
    	{
    		...
    		newboardstate[tempc.x][tempc.y].setPiece(newboardstate[fromcell.x][fromcell.y].getpiece(), false);
		...
		if (previous==null)
		{
			if(c.getpiece()!=null)
			{
				...
				if(c.getpiece() instanceof King) {
					...
					if(chance == 0 && ((King)c.getpiece()).canRoque(boardState, wr02)) {
						destinationlist.add(boardState[7][7]);
					}
				}
				else
				{
		...
		else
		{
			...
			else if(c.getpiece()==null||previous.getpiece().getcolor()!=c.getpiece().getcolor())
			{
				if(c.ispossibledestination())
				{
					if(c.getpiece()!=null)
						c.removePiece();
					int lastX = previous.getpiece().getcolor() == 0 ? 0 : 7;
					if(previous.getpiece() instanceof Pawn && lastX == c.x) {
						((Pawn)previous.getpiece()).promote(c, this);
					} else {
			...
			{
				if(chance == 0 && previous.getpiece() instanceof King && ((King)previous.getpiece()).canRoque(boardState, wr02) && c.x == 7 && c.y == 7) {
					boardState[7][6].setPiece(previous.getpiece());
					boardState[7][5].setPiece(c.getpiece());
					previous.removePiece();
					c.removePiece();	
